...
#        zero_crossings = np.where(np.diff(np.sign(derivative_line_gap)) == -2)[0]+1
        zero_crossings = np.where(np.sign(derivative_line_gap) == -1)[0]
...
class Paragraph:
    ...
    def add_content(self, content, type):
        ...
        elif type==ContentType.Table and isinstance(content, DataFrame):
            self.content.append({'type': type, 'content': content})
        elif type==ContentType.Table:
            table=content.to_pandas()
            self.content.append({'type': type, 'content': table,'top': content.top, 'bottom': content.bottom})

            if len(self.content) >=2 and self.content[-1]['type']== ContentType.Table and self.content[-2]['type']==ContentType.Table:
                if 'top' in self.content[-1] and 'bottom' in self.content[-2] and self.content[-1]['top']-self.content[-2]['bottom'] < 2*content.interline :
                    table2 = self.content[-1]['content']
                    table1 = self.content[-2]['content']

                    if len(table1.columns)==len(table2.columns):
                        try:
                            table1=table2.columns.to_frame().T.append(table2, ignore_index=True)
